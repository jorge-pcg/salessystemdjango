...
function editarProducto(id, codigo, nombre, descripcion, costo, precio, cantidad) {
  document.getElementById("id_producto_editar").value = id;
  document.getElementById("codigo_editar").value = codigo;
...
function imprimirPdf(url, id, iva) {
  // window.open("{% url 'ExportPDF' %}" + id + "/" + iva, '_blank');
  // const url = `url/export/id/iva`
  // const formattedUrl = url.replace('id', id).replace('iva', iva).replace('url', url);
  nueva_url = url + '/export/' + id + '/' + iva
  window.open(nueva_url, '_blank');
}

function editarPersonal(id, codigo, nombre, apellido, telefono, direccion, nif) {
  ...
  document.getElementById("nif_editar").value = nif;
}
